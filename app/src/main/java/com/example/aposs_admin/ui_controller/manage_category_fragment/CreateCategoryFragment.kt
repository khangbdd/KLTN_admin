...
class CreateCategoryFragment : Fragment(), CategoryAndSubCategoryImageAdapter.OnClickListener {
            ...
            if (it != LoadingStatus.Init){
                if (it == LoadingStatus.Loading) {
                    ...
                    if (it == LoadingStatus.Success){
                        ...
                        if(args.id == -1L){
                            Toast.makeText(this.requireContext(), "Tạo danh mục thất bại", Toast.LENGTH_SHORT)
                                .show()
                        }else{
        ...
        if (checkValidEditText(binding.categoryName)
            ...
            if (currentCategory.images.isNotEmpty()) {
                ...
                currentCategory.images.stream().forEach { image ->
                    val imgId =
                        System.currentTimeMillis().toString() + "." + getFileExtension(image.imageUri as Uri)
                ...
                Toast.makeText(this.requireContext(), "Vui lòng chọn ít nhất 1 hình ảnh cho danh mục", Toast.LENGTH_LONG).show()
            }
        } else {
    ...
    private fun openGallery() {
        if(currentCategory.images.size >= maxImageSupport){
            Toast.makeText(this.requireContext(), "Chỉ có thể thêm tối đa 5 ảnh!", Toast.LENGTH_SHORT).show()
        }else{
    ...
    override fun removeImageClick(position: Int) {
        if(currentCategory.images.size > 1){
            ...
            Toast.makeText(this.requireContext(), "Danh mục phải có ít nhất 1 bức ảnh!", Toast.LENGTH_SHORT).show()
        }
    }
